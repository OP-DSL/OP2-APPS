...
void op2_k_aero_6_dotPV_wrapper(
...
const char op2_k_aero_6_dotPV_src[] = R"_op2_k(
namespace op2_m_aero_6_dotPV {
...
__global__
static void op2_k_aero_6_dotPV_init_gbls(
    double *gbl2,
    int stride
) {
    namespace kernel = op2_m_aero_6_dotPV;
...
void op_par_loop_aero_6_dotPV(
    ...
    // op_timing2_enter_kernel("aero_6_dotPV", "c_CUDA", "Direct");
    // op_timing2_enter("Init");

    // op_timing2_enter("Kernel Info Setup");

    static bool first_invocation = true;
    static op::f2c::KernelInfo info("op2_k_aero_6_dotPV_wrapper",
                                    (void *)op2_k_aero_6_dotPV_wrapper,
                                    op2_k_aero_6_dotPV_src);

    auto [block_limit, block_size] = info.get_launch_config(nullptr, set->size);
    block_limit = std::min(block_limit, getBlockLimit(args, n_args, block_size, "aero_6_dotPV"));
    ...
    op2_k_aero_6_dotPV_init_gbls<<<max_blocks, block_size>>>(
        (double *)arg2.data_d,
        stride_gbl
    );
