...
__global__ void op_cuda_aero_1_res_calc(
    ...
    if (thread_id + start < end) {
        ...
        double arg4_0_local[4];
        for (int d = 0; optflags & 1 << 3 && d < 4; ++d)
            arg4_0_local[d] = ZERO_double;

        double arg4_1_local[4];
        for (int d = 0; optflags & 1 << 3 && d < 4; ++d)
            arg4_1_local[d] = ZERO_double;

        double arg4_2_local[4];
        for (int d = 0; optflags & 1 << 3 && d < 4; ++d)
            arg4_2_local[d] = ZERO_double;

        double arg4_3_local[4];
        for (int d = 0; optflags & 1 << 3 && d < 4; ++d)
            arg4_3_local[d] = ZERO_double;
        ...
        double *arg3_vec[4];
    if (optflags & 1 << 1) {
            arg3_vec[0] = dat3 + map0[round32(set_size) * 0 + n] * 1;
            arg3_vec[1] = dat3 + map0[round32(set_size) * 1 + n] * 1;
            arg3_vec[2] = dat3 + map0[round32(set_size) * 2 + n] * 1;
            arg3_vec[3] = dat3 + map0[round32(set_size) * 3 + n] * 1;
    }

        double *arg4_vec[4];
    if (optflags & 1 << 2) {
            arg4_vec[0] = arg4_0_local;
            arg4_vec[1] = arg4_1_local;
            arg4_vec[2] = arg4_2_local;
            arg4_vec[3] = arg4_3_local;
    }

        op2_k1::res_calc(
            arg0_vec,
            arg1_vec,
            optflags & 1 << 0 ? dat2 + n * 16 : NULL,
            optflags & 1 << 1 ? arg3_vec : NULL,
            optflags & 1 << 2 ? arg4_vec : NULL
        );

        for (int d = 0; optflags & 1 << 3 && d < 4; ++d)
            atomicAdd(dat4 + map0[round32(set_size) * 0 + n] * 4 + d, arg4_0_local[d]);

        for (int d = 0; optflags & 1 << 3 && d < 4; ++d)
            atomicAdd(dat4 + map0[round32(set_size) * 1 + n] * 4 + d, arg4_1_local[d]);

        for (int d = 0; optflags & 1 << 3 && d < 4; ++d)
            atomicAdd(dat4 + map0[round32(set_size) * 2 + n] * 4 + d, arg4_2_local[d]);

        for (int d = 0; optflags & 1 << 3 && d < 4; ++d)
            atomicAdd(dat4 + map0[round32(set_size) * 3 + n] * 4 + d, arg4_3_local[d]);
    }
}
...
void op_par_loop_aero_1_res_calc(
    ...
    for (int round = 0; round < 2; ++round ) {
        ...
        if (end - start > 0) {
            ...
            op_cuda_aero_1_res_calc<<<num_blocks, block_size>>>(
                optflags,
                (double *)arg0.data_d,
                (double *)arg1.data_d,
                (double *)arg2.data_d,
