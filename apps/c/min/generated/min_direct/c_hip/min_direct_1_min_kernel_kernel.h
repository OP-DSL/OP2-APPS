...
void op2_k_min_direct_1_min_kernel_wrapper(
...
const char op2_k_min_direct_1_min_kernel_src[] = R"_op2_k(
namespace op2_m_min_direct_1_min_kernel {
...
__global__
static void op2_k_min_direct_1_min_kernel_init_gbls(
    int *gbl1,
    int *gbl1_ref,
    int stride
) {
    namespace kernel = op2_m_min_direct_1_min_kernel;
...
void op_par_loop_min_direct_1_min_kernel(
    ...
    // op_timing2_enter_kernel("min_direct_1_min_kernel", "c_CUDA", "Direct");
    // op_timing2_enter("Init");

    // op_timing2_enter("Kernel Info Setup");

    static bool first_invocation = true;
    static op::f2c::KernelInfo info("op2_k_min_direct_1_min_kernel_wrapper",
                                    (void *)op2_k_min_direct_1_min_kernel_wrapper,
                                    op2_k_min_direct_1_min_kernel_src);

    auto [block_limit, block_size] = info.get_launch_config(nullptr, set->size);
    block_limit = std::min(block_limit, getBlockLimit(args, n_args, block_size, "min_direct_1_min_kernel"));
    ...
    op2_k_min_direct_1_min_kernel_init_gbls<<<max_blocks, block_size>>>(
        (int *)arg1.data_d,
        gbl1_ref_d,
        stride_gbl
    );
