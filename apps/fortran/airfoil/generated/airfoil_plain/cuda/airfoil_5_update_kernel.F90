...
SUBROUTINE update(qold, q, res, adt, rms, maxerr, idx, errloc)
  ...
  DO i = 1, 4
    ...
    q(i) = qold(i) - del
    res(i) = 0.0_8
    rms(op2_s(2, gbl)) = rms(op2_s(2, gbl)) + del ** 2
...
attributes(global) &
subroutine op2_k_airfoil_5_update_init_gbls( &
    gbl4, &
    gbl5, &
    gbl5_ref, &
    dummy &
    ...
    do d = 1, 2
        gbl4(thread_id + (d - 1) * op2_stride_gbl_d) = 0
    end do

    do d = 1, 1
        gbl5(thread_id + (d - 1) * op2_stride_gbl_d) = gbl5_ref(d)
    end do
end subroutine
...
subroutine op2_k_airfoil_5_update_main( &
    ...
    real(8), dimension(:), pointer :: gbl5
    real(8), dimension(:), pointer, device :: gbl5_d
    real(8), dimension(:), allocatable, device, save :: gbl5_ref_d

    integer(4), dimension(:), pointer :: info7
    integer(4), dimension(:), pointer, device :: info7_d
    ...
    call op_timing2_enter_kernel("airfoil_5_update", "CUDA", "Direct")
    ...
    call c_f_pointer(arg5%data_d, gbl5_d, (/1 * block_size * max_blocks/))

    call c_f_pointer(arg7%data, info7, (/1/))
    call c_f_pointer(arg7%data_d, info7_d, (/1 * block_size * max_blocks/))
    ...
    if (.not. allocated(gbl5_ref_d)) then
        allocate(gbl5_ref_d(1))
    end if

    gbl5_ref_d = gbl5

    call op_timing2_enter("Init GBLs")
    call op2_k_airfoil_5_update_init_gbls<<<max_blocks, block_size>>>( &
        gbl4_d, &
        gbl5_d, &
        gbl5_ref_d, &
        0 &
    ...
    if (err /= 0) then
        print *, "error in gpu kernel: ", "airfoil_5_update"
        print *, cudaGetErrorString(err)
    end if
