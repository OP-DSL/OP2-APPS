...
SUBROUTINE bres_calc_simd(x1, x2, q1, adt1, res1, bound)
  ...
  IF (bound == 1) THEN
    res1(op2_s(2, SIMD_LEN)) = res1(op2_s(2, SIMD_LEN)) + p1 * dy
    res1(op2_s(3, SIMD_LEN)) = res1(op2_s(3, SIMD_LEN)) - p1 * dx
    RETURN
  END IF
  ...
  f = 0.5_8 * (vol1 * q1(op2_s(1, SIMD_LEN)) + vol2 * op2_const_qinf(1)) + mu * (q1(op2_s(1, SIMD_LEN)) - op2_const_qinf(1))
  res1(op2_s(1, SIMD_LEN)) = res1(op2_s(1, SIMD_LEN)) + f
  f = 0.5_8 * (vol1 * q1(op2_s(2, SIMD_LEN)) + p1 * dy + vol2 * op2_const_qinf(2) + p2 * dy) + mu * (q1(op2_s(2, SIMD_LEN)) - op2_const_qinf(2))
  res1(op2_s(2, SIMD_LEN)) = res1(op2_s(2, SIMD_LEN)) + f
  f = 0.5_8 * (vol1 * q1(op2_s(3, SIMD_LEN)) - p1 * dx + vol2 * op2_const_qinf(3) - p2 * dx) + mu * (q1(op2_s(3, SIMD_LEN)) - op2_const_qinf(3))
  res1(op2_s(3, SIMD_LEN)) = res1(op2_s(3, SIMD_LEN)) + f
  f = 0.5_8 * (vol1 * (q1(op2_s(4, SIMD_LEN)) + p1) + vol2 * (op2_const_qinf(4) + p2)) + mu * (q1(op2_s(4, SIMD_LEN)) - op2_const_qinf(4))
...
subroutine bres_calc_wrapper2( &
    ...
    real(8), dimension(SIMD_LEN, 4) :: arg4_local
    ...
    do while (block + SIMD_LEN <= end)
        ...
        do lane = 1, SIMD_LEN
            ...
            arg3_local(lane, :) = dat2(:, map1(1, n + 1) + 1)
        end do
        ...
        do lane = 1, SIMD_LEN
            ...
            dat3(:, map1(1, n + 1) + 1) = dat3(:, map1(1, n + 1) + 1) + arg4_local(lane, :)
        end do
