...
void op2_k_jac_1_res_wrapper(
    ...
    for (int i = thread_id + start; i < end; i += blockDim.x * gridDim.x) {
        ...
        op2_m_jac_1_res::res(
            dat0 + n * 3,
            dat1 + map0[1 * stride + n] * 2,
            arg2_0_local,
            &gbl3
        );

        for (int d = 0; d < 3; ++d)
            atomicAdd(dat2 + map0[0 * stride + n] * 3 + d, arg2_0_local[d]);
    }
}


const char op2_k_jac_1_res_src[] = R"_op2_k(
namespace op2_m_jac_1_res {
...
void op_par_loop_jac_1_res(
    ...
    namespace kernel = op2_m_jac_1_res;
    ...
    op_profile_enter_kernel("jac_1_res", "c_CUDA", "Indirect (atomics)");
    op_profile_enter("Init");

    op_profile_enter("Kernel Info Setup");

    static bool first_invocation = true;
    static op::f2c::KernelInfo info("op2_k_jac_1_res_wrapper",
                                    (void *)op2_k_jac_1_res_wrapper,
                                    op2_k_jac_1_res_src);
