namespace op2_m_reduction_2_update {
...
void op2_k_reduction_2_update_wrapper(
    ...
    for (int i = thread_id + start; i < end; i += blockDim.x * gridDim.x) {
        ...
        op2_m_reduction_2_update::update(
            dat0 + n * 4,
            gbl1 + thread_id
        );
    }
...
const char op2_k_reduction_2_update_src[] = R"_op2_k(
namespace op2_m_reduction_2_update {
...
__global__
static void op2_k_reduction_2_update_init_gbls(
    int *gbl1,
    int stride
) {
    namespace kernel = op2_m_reduction_2_update;
...
void op_par_loop_reduction_2_update(
    ...
    // op_timing2_enter_kernel("reduction_2_update", "c_CUDA", "Direct");
    // op_timing2_enter("Init");

    // op_timing2_enter("Kernel Info Setup");

    static bool first_invocation = true;
    static op::f2c::KernelInfo info("op2_k_reduction_2_update_wrapper",
                                    (void *)op2_k_reduction_2_update_wrapper,
                                    op2_k_reduction_2_update_src);

    auto [block_limit, block_size] = info.get_launch_config(nullptr, set->size);
    block_limit = std::min(block_limit, getBlockLimit(args, n_args, block_size, "reduction_2_update"));
    ...
    op2_k_reduction_2_update_init_gbls<<<max_blocks, block_size>>>(
        (int *)arg1.data_d,
        stride_gbl
    );
