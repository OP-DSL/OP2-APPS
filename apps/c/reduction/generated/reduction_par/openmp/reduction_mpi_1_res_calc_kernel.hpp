...
void reduction_mpi_1_res_calc_wrapper(
    ...
    for (; block + SIMD_LEN < end; block += SIMD_LEN) {
        alignas(SIMD_LEN * 8) double arg0_0_local[SIMD_LEN][4] = {0};
        ...
        for (int lane = 0; lane < SIMD_LEN; ++lane) {
            ...
            for (int d = 0; d < 4; ++d) {
                (dat0 + map0[n * map0_dim + 0] * 4)[d] += arg0_0_local[lane][d];
            }
...
void op_par_loop_reduction_mpi_1_res_calc(
    ...
    for (int col = 0; col < plan->ncolors; ++col) {
        ...
        for (int block_idx = 0; block_idx < num_blocks; ++block_idx) {
            ...
            reduction_mpi_1_res_calc_wrapper(
                (double *)arg0.data,
                arg0.map_data,
                arg0.map->dim,
