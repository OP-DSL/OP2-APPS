
#define SIMD_LEN 8
#define op2_s(comp, simd_len) ((comp-1)*simd_len + 1)

module op2_m_reduction_2_edge_count_m

    use iso_c_binding
    use omp_lib

    use op2_fortran_declarations
    use op2_fortran_rt_support

    use op2_consts

    implicit none

    private
    public :: op2_k_reduction_2_edge_count_m

contains

SUBROUTINE edge_count_simd(res, edge_count_result)
  IMPLICIT NONE
  REAL(KIND = 8), DIMENSION(*), INTENT(OUT) :: res
  INTEGER(KIND = 4), INTENT(OUT) :: edge_count_result
  INTEGER(KIND = 4) :: d
  DO d = 1, 4
    res(op2_s(d, SIMD_LEN)) = 0.0
  END DO
  edge_count_result = edge_count_result + 1
END SUBROUTINE

SUBROUTINE edge_count(res, edge_count_result)
  IMPLICIT NONE
  REAL(KIND = 8), DIMENSION(4), INTENT(OUT) :: res
  INTEGER(KIND = 4), INTENT(OUT) :: edge_count_result
  INTEGER(KIND = 4) :: d
  DO d = 1, 4
    res(d) = 0.0
  END DO
  edge_count_result = edge_count_result + 1
END SUBROUTINE

subroutine edge_count_wrapper2( &
    dat0, &
    map0, &
    gbl1, &
    start, &
    end &
)
    implicit none

    ! parameters
    real(8), dimension(4, *) :: dat0

    integer(4), dimension(:, :) :: map0

    integer(4), dimension(1) :: gbl1

    integer(4) :: start, end

    ! locals
    integer(4) :: n
    integer(4) :: block, lane, d

    real(8), dimension(SIMD_LEN, 4) :: arg0_local
    integer(4), dimension(SIMD_LEN, 1) :: arg1_local

    block = start
    do while (block + SIMD_LEN <= end)
        arg1_local = 0

        do lane = 1, SIMD_LEN
            n = block + lane - 1

            arg0_local(lane, :) = dat0(:, map0(1, n + 1) + 1)
        end do

        !$omp simd
        do lane = 1, SIMD_LEN
            n = block + lane - 1

            call edge_count_simd( &
                arg0_local(lane, 1), &
                arg1_local(lane, 1) &
            )
        end do

        ! Reduction back to globals
        do lane = 1, SIMD_LEN
            n = block + lane - 1

            dat0(:, map0(1, n + 1) + 1) = arg0_local(lane, :)
            gbl1 = gbl1 + arg1_local(lane, :)
        end do

        block = block + SIMD_LEN
    end do

    do n = block, end
        call edge_count( &
            dat0(:, map0(1, n + 1) + 1), &
            gbl1(1) &
        )
    end do
end subroutine

subroutine edge_count_wrapper( &
    name, &
    dat0, &
    map0, &
    gbl1, &
    set, &
    args, &
    num_dats_indirect, &
    dats_indirect &
)
    implicit none

    ! parameters
    character(kind=c_char, len=*) :: name

    real(8), dimension(4, *) :: dat0

    integer(4), dimension(:, :) :: map0

    integer(4), dimension(1) :: gbl1

    type(op_set) :: set
    type(op_arg), dimension(2) :: args

    integer(4) :: num_dats_indirect
    integer(4), dimension(2) :: dats_indirect

    ! locals
    integer(4) :: thread, start, end, n
    integer(4) :: num_threads

    integer(4) :: part_size, col, block_idx, block_offset, num_blocks, block_id, num_elem, offset

    type(op_plan), pointer :: plan
    integer(4), dimension(:), pointer :: plan_ncolblk, plan_blkmap, plan_nelems, plan_offset

    integer(4), dimension(:), allocatable :: gbl1_temp

#ifdef OP_PART_SIZE_2
    part_size = OP_PART_SIZE_2
#else
    part_size = 0
#endif

    plan => fortranplancaller( &
        name // c_null_char, &
        set%setcptr, &
        part_size, &
        size(args), &
        args, &
        num_dats_indirect, &
        dats_indirect, &
        2 &
    )

    call c_f_pointer(plan%ncolblk, plan_ncolblk, (/ plan%ncolors /))
    call c_f_pointer(plan%blkmap, plan_blkmap, (/ plan%nblocks /))
    call c_f_pointer(plan%nelems, plan_nelems, (/ plan%nblocks /))
    call c_f_pointer(plan%offset, plan_offset, (/ plan%nblocks /))

    num_threads = omp_get_max_threads()

    allocate(gbl1_temp(num_threads * 64))
    gbl1_temp = 0

    block_offset = 0
    do col = 1, plan%ncolors
        if (col == plan%ncolors_core + 1) then
            call op_mpi_wait_all(size(args), args)
        end if

        num_blocks = plan_ncolblk(col)

        !$omp parallel do private(thread, block_idx, block_id, num_elem, offset, n)
        do block_idx = 1, num_blocks
            thread = omp_get_thread_num() + 1

            block_id = plan_blkmap(block_idx + block_offset) + 1
            num_elem = plan_nelems(block_id)
            offset = plan_offset(block_id)

            call edge_count_wrapper2( &
                dat0, &
                map0, &
                gbl1_temp(omp_get_thread_num() * 64 + 1), &
                offset, &
                offset + num_elem - 1 &
            )
        end do

        block_offset = block_offset + num_blocks

        if (col /= plan%ncolors_owned) then
            cycle
        end if

        do thread = 1, num_threads
            start = (thread - 1) * 64 + 1
            gbl1 = gbl1 + gbl1_temp(start : start + 0)
        end do
    end do
end subroutine

subroutine op2_k_reduction_2_edge_count_m( &
    name, &
    set, &
    arg0, &
    arg1 &
)
    implicit none

    ! parameters
    character(kind=c_char, len=*) :: name
    type(op_set) :: set

    type(op_arg) :: arg0
    type(op_arg) :: arg1

    ! locals
    type(op_arg), dimension(2) :: args

    integer(4) :: num_dats_indirect
    integer(4), dimension(2) :: dats_indirect

    integer(4) :: set_size

    real(8), pointer, dimension(:, :) :: dat0

    integer(4), pointer, dimension(:, :) :: map0

    integer(4), pointer, dimension(:) :: gbl1


    args(1) = arg0
    args(2) = arg1

    num_dats_indirect = 1
    dats_indirect = (/0, -1/)

    call op_profile_enter_kernel("reduction_2_edge_count", "openmp", "Indirect")

    call op_profile_enter("MPI Exchanges")
    set_size = op_mpi_halo_exchanges(set%setcptr, size(args), args)

    call op_profile_next("Computation")

    call c_f_pointer(arg0%data, dat0, (/4, getsetsizefromoparg(arg0)/))

    call c_f_pointer(arg0%map_data, map0, (/getmapdimfromoparg(arg0), set%setptr%size/))

    call c_f_pointer(arg1%data, gbl1, (/1/))

    call edge_count_wrapper( &
        name, &
        dat0, &
        map0, &
        gbl1, &
        set, &
        args, &
        num_dats_indirect, &
        dats_indirect &
    )

    call op_profile_next("MPI Wait")
    if ((set_size .eq. 0) .or. (set_size .eq. set%setptr%core_size)) then
        call op_mpi_wait_all(size(args), args)
    end if

    call op_mpi_reduce_int(arg1, arg1%data)

    call op_profile_exit()

    call op_mpi_set_dirtybit(size(args), args)
    call op_profile_exit()
end subroutine

end module

module op2_m_reduction_2_edge_count_fb

    use iso_c_binding

    use op2_fortran_declarations
    use op2_fortran_rt_support

    use op2_consts

    implicit none

    private
    public :: op2_k_reduction_2_edge_count_fb

contains

SUBROUTINE edge_count(res, edge_count_result)
  IMPLICIT NONE
  REAL(KIND = 8), DIMENSION(4), INTENT(OUT) :: res
  INTEGER(KIND = 4), INTENT(OUT) :: edge_count_result
  INTEGER(KIND = 4) :: d
  DO d = 1, 4
    res(d) = 0.0
  END DO
  edge_count_result = edge_count_result + 1
END SUBROUTINE

subroutine op2_k_reduction_2_edge_count_wr( &
    dat0, &
    map0, &
    gbl1, &
    n_exec, &
    set, &
    args &
)
    implicit none

    ! parameters
    real(8), dimension(:, :) :: dat0

    integer(4), dimension(:, :) :: map0

    integer(4), dimension(:) :: gbl1

    integer(4) :: n_exec
    type(op_set) :: set
    type(op_arg), dimension(2) :: args

    ! locals
    integer(4), dimension(size(gbl1)) :: gbl1_temp

    integer(4) :: n

    gbl1_temp = gbl1

    do n = 1, n_exec
        if (n == set%setptr%core_size + 1) then
            call op_profile_next("MPI Wait")
            call op_mpi_wait_all(size(args), args)
            call op_profile_next("Computation")
        end if

        if (n == set%setptr%size + 1) then
            gbl1 = gbl1_temp
        end if

        call edge_count( &
            dat0(:, map0(1, n) + 1), &
            gbl1_temp(1) &
        )
    end do

    if (n_exec <= set%setptr%size) then
        gbl1 = gbl1_temp
    end if
end subroutine

subroutine op2_k_reduction_2_edge_count_fb( &
    name, &
    set, &
    arg0, &
    arg1 &
)
    implicit none

    ! parameters
    character(kind=c_char, len=*) :: name
    type(op_set) :: set

    type(op_arg) :: arg0
    type(op_arg) :: arg1

    ! locals
    type(op_arg), dimension(2) :: args

    integer(4) :: n_exec

    real(8), pointer, dimension(:, :) :: dat0

    integer(4), pointer, dimension(:, :) :: map0

    integer(4), pointer, dimension(:) :: gbl1

    real(4) :: transfer

    args(1) = arg0
    args(2) = arg1

    call op_profile_enter_kernel("reduction_2_edge_count", "seq", "Indirect")

    call op_profile_enter("MPI Exchanges")
    n_exec = op_mpi_halo_exchanges(set%setcptr, size(args), args)

    call op_profile_next("Computation")

    call c_f_pointer(arg0%data, dat0, (/4, getsetsizefromoparg(arg0)/))

    call c_f_pointer(arg0%map_data, map0, (/getmapdimfromoparg(arg0), set%setptr%size/))

    call c_f_pointer(arg1%data, gbl1, (/1/))

    call op2_k_reduction_2_edge_count_wr( &
        dat0, &
        map0, &
        gbl1, &
        n_exec, &
        set, &
        args &
    )

    call op_profile_next("MPI Wait")
    if ((n_exec == 0) .or. (n_exec == set%setptr%core_size)) then
        call op_mpi_wait_all(size(args), args)
    end if

    call op_profile_next("MPI Reduce")

    call op_mpi_reduce_int(arg1, arg1%data)

    call op_profile_exit()

    call op_mpi_set_dirtybit(size(args), args)
    call op_profile_exit()
end subroutine

end module

module op2_m_reduction_2_edge_count

    use iso_c_binding

    use op2_fortran_declarations
    use op2_fortran_rt_support

    use op2_m_reduction_2_edge_count_fb
    use op2_m_reduction_2_edge_count_m

    implicit none

    private
    public :: op2_k_reduction_2_edge_count

contains

subroutine op2_k_reduction_2_edge_count( &
    name, &
    set, &
    arg0, &
    arg1 &
)
    character(kind=c_char, len=*) :: name
    type(op_set) :: set

    type(op_arg) :: arg0
    type(op_arg) :: arg1

    if (op_check_whitelist("reduction_2_edge_count")) then
        call op2_k_reduction_2_edge_count_m( &
            name, &
            set, &
            arg0, &
            arg1 &
        )
    else
        call op_check_fallback_mode("reduction_2_edge_count")
        call op2_k_reduction_2_edge_count_fb( &
            name, &
            set, &
            arg0, &
            arg1 &
        )
    end if

end subroutine

end module