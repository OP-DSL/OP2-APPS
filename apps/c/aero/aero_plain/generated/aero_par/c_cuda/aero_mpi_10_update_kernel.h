...
extern "C" __global__ 
void op2_k_aero_mpi_10_update_wrapper(
...
const char op2_k_aero_mpi_10_update_src[] = R"_op2_k(
namespace op2_m_aero_mpi_10_update {
...
__global__
static void op2_k_aero_mpi_10_update_init_gbls(
    double *gbl3,
    int stride
) {
    namespace kernel = op2_m_aero_mpi_10_update;
...
void op_par_loop_aero_mpi_10_update(
    ...
    // op_timing2_enter_kernel("aero_mpi_10_update", "c_CUDA", "Direct");
    // op_timing2_enter("Init");

    // op_timing2_enter("Kernel Info Setup");

    static bool first_invocation = true;
    static op::f2c::KernelInfo info("op2_k_aero_mpi_10_update_wrapper",
                                    (void *)op2_k_aero_mpi_10_update_wrapper,
                                    op2_k_aero_mpi_10_update_src);

    auto [block_limit, block_size] = info.get_launch_config(nullptr, set->size);
    block_limit = std::min(block_limit, getBlockLimit(args, n_args, block_size, "aero_mpi_10_update"));
    ...
    op2_k_aero_mpi_10_update_init_gbls<<<max_blocks, block_size>>>(
        (double *)arg3.data_d,
        stride_gbl
    );
