...
void op2_k_aero_mpi_8_dotR_wrapper(
...
const char op2_k_aero_mpi_8_dotR_src[] = R"_op2_k(
namespace op2_m_aero_mpi_8_dotR {
...
__global__
static void op2_k_aero_mpi_8_dotR_init_gbls(
    double *gbl1,
    int stride
) {
    namespace kernel = op2_m_aero_mpi_8_dotR;
...
void op_par_loop_aero_mpi_8_dotR(
    ...
    // op_timing2_enter_kernel("aero_mpi_8_dotR", "c_CUDA", "Direct");
    // op_timing2_enter("Init");

    // op_timing2_enter("Kernel Info Setup");

    static bool first_invocation = true;
    static op::f2c::KernelInfo info("op2_k_aero_mpi_8_dotR_wrapper",
                                    (void *)op2_k_aero_mpi_8_dotR_wrapper,
                                    op2_k_aero_mpi_8_dotR_src);

    auto [block_limit, block_size] = info.get_launch_config(nullptr, set->size);
    block_limit = std::min(block_limit, getBlockLimit(args, n_args, block_size, "aero_mpi_8_dotR"));
    ...
    op2_k_aero_mpi_8_dotR_init_gbls<<<max_blocks, block_size>>>(
        (double *)arg1.data_d,
        stride_gbl
    );
