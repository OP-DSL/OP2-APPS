namespace op2_m_airfoil_step7_5_update {
...
void op2_k_airfoil_step7_5_update_wrapper(
    ...
    for (int i = thread_id + start; i < end; i += blockDim.x * gridDim.x) {
        ...
        op2_m_airfoil_step7_5_update::update(
            dat0 + n * 4,
            dat1 + n * 4,
            dat2 + n * 4,
            dat3 + n * 1,
...
const char op2_k_airfoil_step7_5_update_src[] = R"_op2_k(
namespace op2_m_airfoil_step7_5_update {
...
__global__
static void op2_k_airfoil_step7_5_update_init_gbls(
    double *gbl4,
    int stride
) {
    namespace kernel = op2_m_airfoil_step7_5_update;
...
void op_par_loop_airfoil_step7_5_update(
    const char *name,
    op_set set,
    op_arg arg0,
    op_arg arg1,
    ...
    // op_timing2_enter_kernel("airfoil_step7_5_update", "c_CUDA", "Direct");
    // op_timing2_enter("Init");

    // op_timing2_enter("Kernel Info Setup");

    static bool first_invocation = true;
    static op::f2c::KernelInfo info("op2_k_airfoil_step7_5_update_wrapper",
                                    (void *)op2_k_airfoil_step7_5_update_wrapper,
                                    op2_k_airfoil_step7_5_update_src);

    auto [block_limit, block_size] = info.get_launch_config(nullptr, set->size);
    block_limit = std::min(block_limit, getBlockLimit(args, n_args, block_size, "airfoil_step7_5_update"));
    ...
    op2_k_airfoil_step7_5_update_init_gbls<<<max_blocks, block_size>>>(
        (double *)arg4.data_d,
        stride_gbl
    );
