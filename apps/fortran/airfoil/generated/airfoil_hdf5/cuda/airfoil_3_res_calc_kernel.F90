...
subroutine op2_k_airfoil_3_res_calc_wrapper( &
    ...
    do n = thread_id + start, end, blockDim%x * gridDim%x
        call res_calc( &
            ...
            dat1(map1(1 * set_size + n) * 4 + 1), &
            dat2(map1(0 * set_size + n) * 1 + 1), &
            dat2(map1(1 * set_size + n) * 1 + 1), &
            dat3(map1(0 * set_size + n) * 4 + 1), &
            dat3(map1(1 * set_size + n) * 4 + 1) &
        )
    end do
end subroutine
