namespace op2_m_aero_mpi_2_dirichlet {
...
void op2_k_aero_mpi_2_dirichlet_wrapper(
    ...
    for (int i = thread_id + start; i < end; i += blockDim.x * gridDim.x) {
        ...
        op2_m_aero_mpi_2_dirichlet::dirichlet(
...
const char op2_k_aero_mpi_2_dirichlet_src[] = R"_op2_k(
namespace op2_m_aero_mpi_2_dirichlet {
...
void op_par_loop_aero_mpi_2_dirichlet(
    ...
    namespace kernel = op2_m_aero_mpi_2_dirichlet;
    ...
    // op_timing2_enter_kernel("aero_mpi_2_dirichlet", "c_CUDA", "Indirect (atomics)");
    // op_timing2_enter("Init");

    // op_timing2_enter("Kernel Info Setup");

    static bool first_invocation = true;
    static op::f2c::KernelInfo info("op2_k_aero_mpi_2_dirichlet_wrapper",
                                    (void *)op2_k_aero_mpi_2_dirichlet_wrapper,
                                    op2_k_aero_mpi_2_dirichlet_src);
    ...
    block_limit = std::min(block_limit, getBlockLimit(args, n_args, block_size, "aero_mpi_2_dirichlet"));
