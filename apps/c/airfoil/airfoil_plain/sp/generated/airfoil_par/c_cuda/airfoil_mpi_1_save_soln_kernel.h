...
__device__ inline void save_soln(const float *q, float *qold) {
  for (int n = 0; n < 4; n++)
    qold[n] = q[n];
}}
...
extern "C" __global__ 
void op2_k_airfoil_mpi_1_save_soln_wrapper(
...
const char op2_k_airfoil_mpi_1_save_soln_src[] = R"_op2_k(
namespace op2_m_airfoil_mpi_1_save_soln {
...
void op_par_loop_airfoil_mpi_1_save_soln(
    ...
    namespace kernel = op2_m_airfoil_mpi_1_save_soln;
    ...
    op_profile_enter_kernel("airfoil_mpi_1_save_soln", "c_CUDA", "Direct");
    op_profile_enter("Init");

    op_profile_enter("Kernel Info Setup");

    static bool first_invocation = true;
    static op::f2c::KernelInfo info("op2_k_airfoil_mpi_1_save_soln_wrapper",
                                    (void *)op2_k_airfoil_mpi_1_save_soln_wrapper,
                                    op2_k_airfoil_mpi_1_save_soln_src);

    auto [block_limit, block_size] = info.get_launch_config(nullptr, set->size);
    block_limit = std::min(block_limit, getBlockLimit(args, n_args, block_size, "airfoil_mpi_1_save_soln"));
