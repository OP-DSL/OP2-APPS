...
__global__ void op_cuda_reduction_mpi_2_update(
...
void op_par_loop_reduction_mpi_2_update(
    ...
    if (OP_diags > 2)
        printf(" kernel routine (direct): reduction_mpi_2_update\n");
    ...
    op_cuda_reduction_mpi_2_update<<<num_blocks, block_size, reduction_size * block_size>>>(
        (double *)arg0.data_d,
        (int *)arg1.data_d,
        set->size
    );
