
#define SIMD_LEN 8
#define op2_s(comp, simd_len) ((comp-1)*simd_len + 1)

module op2_m_airfoil_2_adt_calc_m

    use iso_c_binding
    use omp_lib

    use op2_fortran_declarations
    use op2_fortran_rt_support

    use op2_consts

    implicit none

    private
    public :: op2_k_airfoil_2_adt_calc_m

contains

SUBROUTINE adt_calc_simd(x1, x2, x3, x4, q, adt)
  REAL(KIND = 8), DIMENSION(*), INTENT(IN) :: x4
  REAL(KIND = 8), DIMENSION(4), INTENT(IN) :: q
  REAL(KIND = 8), INTENT(OUT) :: adt
  REAL(KIND = 8) :: dx, dy, ri, u, v, c
  REAL(KIND = 8), DIMENSION(*), INTENT(IN) :: x1
  REAL(KIND = 8), DIMENSION(*), INTENT(IN) :: x2
  REAL(KIND = 8), DIMENSION(*), INTENT(IN) :: x3
  ri = 1.0_8 / q(1)
  u = ri * q(2)
  v = ri * q(3)
  c = SQRT(op2_const_gam * op2_const_gm1 * (ri * q(4) - 0.5_8 * (u ** 2 + v ** 2)))
  dx = x2(op2_s(1, SIMD_LEN)) - x1(op2_s(1, SIMD_LEN))
  dy = x2(op2_s(2, SIMD_LEN)) - x1(op2_s(2, SIMD_LEN))
  adt = ABS(u * dy - v * dx) + c * SQRT(dx ** 2 + dy ** 2)
  dx = x3(op2_s(1, SIMD_LEN)) - x2(op2_s(1, SIMD_LEN))
  dy = x3(op2_s(2, SIMD_LEN)) - x2(op2_s(2, SIMD_LEN))
  adt = adt + ABS(u * dy - v * dx) + c * SQRT(dx ** 2 + dy ** 2)
  dx = x4(op2_s(1, SIMD_LEN)) - x3(op2_s(1, SIMD_LEN))
  dy = x4(op2_s(2, SIMD_LEN)) - x3(op2_s(2, SIMD_LEN))
  adt = adt + ABS(u * dy - v * dx) + c * SQRT(dx ** 2 + dy ** 2)
  dx = x1(op2_s(1, SIMD_LEN)) - x4(op2_s(1, SIMD_LEN))
  dy = x1(op2_s(2, SIMD_LEN)) - x4(op2_s(2, SIMD_LEN))
  adt = adt + ABS(u * dy - v * dx) + c * SQRT(dx ** 2 + dy ** 2)
  adt = adt / op2_const_cfl
END SUBROUTINE

SUBROUTINE adt_calc(x1, x2, x3, x4, q, adt)
  REAL(KIND = 8), DIMENSION(2), INTENT(IN) :: x1, x2, x3, x4
  REAL(KIND = 8), DIMENSION(4), INTENT(IN) :: q
  REAL(KIND = 8), INTENT(OUT) :: adt
  REAL(KIND = 8) :: dx, dy, ri, u, v, c
  ri = 1.0_8 / q(1)
  u = ri * q(2)
  v = ri * q(3)
  c = SQRT(op2_const_gam * op2_const_gm1 * (ri * q(4) - 0.5_8 * (u ** 2 + v ** 2)))
  dx = x2(1) - x1(1)
  dy = x2(2) - x1(2)
  adt = ABS(u * dy - v * dx) + c * SQRT(dx ** 2 + dy ** 2)
  dx = x3(1) - x2(1)
  dy = x3(2) - x2(2)
  adt = adt + ABS(u * dy - v * dx) + c * SQRT(dx ** 2 + dy ** 2)
  dx = x4(1) - x3(1)
  dy = x4(2) - x3(2)
  adt = adt + ABS(u * dy - v * dx) + c * SQRT(dx ** 2 + dy ** 2)
  dx = x1(1) - x4(1)
  dy = x1(2) - x4(2)
  adt = adt + ABS(u * dy - v * dx) + c * SQRT(dx ** 2 + dy ** 2)
  adt = adt / op2_const_cfl
END SUBROUTINE

subroutine adt_calc_wrapper2( &
    dat0, &
    dat1, &
    dat2, &
    map0, &
    start, &
    end &
)
    implicit none

    ! parameters
    real(8), dimension(2, *) :: dat0
    real(8), dimension(4, *) :: dat1
    real(8), dimension(1, *) :: dat2

    integer(4), dimension(:, :) :: map0

    integer(4) :: start, end

    ! locals
    integer(4) :: n
    integer(4) :: block, lane, d

    real(8), dimension(SIMD_LEN, 2) :: arg0_local
    real(8), dimension(SIMD_LEN, 2) :: arg1_local
    real(8), dimension(SIMD_LEN, 2) :: arg2_local
    real(8), dimension(SIMD_LEN, 2) :: arg3_local

    block = start
    do while (block + SIMD_LEN <= end)

        do lane = 1, SIMD_LEN
            n = block + lane - 1

            arg0_local(lane, :) = dat0(:, map0(1, n + 1) + 1)
            arg1_local(lane, :) = dat0(:, map0(2, n + 1) + 1)
            arg2_local(lane, :) = dat0(:, map0(3, n + 1) + 1)
            arg3_local(lane, :) = dat0(:, map0(4, n + 1) + 1)
        end do

        !$omp simd
        do lane = 1, SIMD_LEN
            n = block + lane - 1

            call adt_calc_simd( &
                arg0_local(lane, 1), &
                arg1_local(lane, 1), &
                arg2_local(lane, 1), &
                arg3_local(lane, 1), &
                dat1(:, n + 1), &
                dat2(1, n + 1) &
            )
        end do

        ! Reduction back to globals
        do lane = 1, SIMD_LEN
            n = block + lane - 1

        end do

        block = block + SIMD_LEN
    end do

    do n = block, end
        call adt_calc( &
            dat0(:, map0(1, n + 1) + 1), &
            dat0(:, map0(2, n + 1) + 1), &
            dat0(:, map0(3, n + 1) + 1), &
            dat0(:, map0(4, n + 1) + 1), &
            dat1(:, n + 1), &
            dat2(1, n + 1) &
        )
    end do
end subroutine

subroutine adt_calc_wrapper( &
    name, &
    dat0, &
    dat1, &
    dat2, &
    map0, &
    set, &
    args, &
    num_dats_indirect, &
    dats_indirect &
)
    implicit none

    ! parameters
    character(kind=c_char, len=*) :: name

    real(8), dimension(2, *) :: dat0
    real(8), dimension(4, *) :: dat1
    real(8), dimension(1, *) :: dat2

    integer(4), dimension(:, :) :: map0

    type(op_set) :: set
    type(op_arg), dimension(6) :: args

    integer(4) :: num_dats_indirect
    integer(4), dimension(6) :: dats_indirect

    ! locals
    integer(4) :: thread, start, end, n
    integer(4) :: num_threads

    integer(4) :: part_size, col, block_idx, block_offset, num_blocks, block_id, num_elem, offset

    type(op_plan), pointer :: plan
    integer(4), dimension(:), pointer :: plan_ncolblk, plan_blkmap, plan_nelems, plan_offset

#ifdef OP_PART_SIZE_2
    part_size = OP_PART_SIZE_2
#else
    part_size = 0
#endif

    plan => fortranplancaller( &
        name // c_null_char, &
        set%setcptr, &
        part_size, &
        size(args), &
        args, &
        num_dats_indirect, &
        dats_indirect, &
        2 &
    )

    call c_f_pointer(plan%ncolblk, plan_ncolblk, (/ plan%ncolors /))
    call c_f_pointer(plan%blkmap, plan_blkmap, (/ plan%nblocks /))
    call c_f_pointer(plan%nelems, plan_nelems, (/ plan%nblocks /))
    call c_f_pointer(plan%offset, plan_offset, (/ plan%nblocks /))

    block_offset = 0
    do col = 1, plan%ncolors
        if (col == plan%ncolors_core + 1) then
            call op_mpi_wait_all(size(args), args)
        end if

        num_blocks = plan_ncolblk(col)

        !$omp parallel do private(thread, block_idx, block_id, num_elem, offset, n)
        do block_idx = 1, num_blocks
            thread = omp_get_thread_num() + 1

            block_id = plan_blkmap(block_idx + block_offset) + 1
            num_elem = plan_nelems(block_id)
            offset = plan_offset(block_id)

            call adt_calc_wrapper2( &
                dat0, &
                dat1, &
                dat2, &
                map0, &
                offset, &
                offset + num_elem - 1 &
            )
        end do

        block_offset = block_offset + num_blocks
    end do
end subroutine

subroutine op2_k_airfoil_2_adt_calc_m( &
    name, &
    set, &
    arg0, &
    arg1, &
    arg2, &
    arg3, &
    arg4, &
    arg5 &
)
    implicit none

    ! parameters
    character(kind=c_char, len=*) :: name
    type(op_set) :: set

    type(op_arg) :: arg0
    type(op_arg) :: arg1
    type(op_arg) :: arg2
    type(op_arg) :: arg3
    type(op_arg) :: arg4
    type(op_arg) :: arg5

    ! locals
    type(op_arg), dimension(6) :: args

    integer(4) :: num_dats_indirect
    integer(4), dimension(6) :: dats_indirect

    integer(4) :: set_size

    real(8), pointer, dimension(:, :) :: dat0
    real(8), pointer, dimension(:, :) :: dat1
    real(8), pointer, dimension(:, :) :: dat2

    integer(4), pointer, dimension(:, :) :: map0


    args(1) = arg0
    args(2) = arg1
    args(3) = arg2
    args(4) = arg3
    args(5) = arg4
    args(6) = arg5

    num_dats_indirect = 1
    dats_indirect = (/0, 0, 0, 0, -1, -1/)

    call op_profile_enter_kernel("airfoil_2_adt_calc", "openmp", "Indirect")

    call op_profile_enter("MPI Exchanges")
    set_size = op_mpi_halo_exchanges(set%setcptr, size(args), args)

    call op_profile_next("Computation")

    call c_f_pointer(arg0%data, dat0, (/2, getsetsizefromoparg(arg0)/))
    call c_f_pointer(arg4%data, dat1, (/4, getsetsizefromoparg(arg4)/))
    call c_f_pointer(arg5%data, dat2, (/1, getsetsizefromoparg(arg5)/))

    call c_f_pointer(arg0%map_data, map0, (/getmapdimfromoparg(arg0), set%setptr%size/))

    call adt_calc_wrapper( &
        name, &
        dat0, &
        dat1, &
        dat2, &
        map0, &
        set, &
        args, &
        num_dats_indirect, &
        dats_indirect &
    )

    call op_profile_next("MPI Wait")
    if ((set_size .eq. 0) .or. (set_size .eq. set%setptr%core_size)) then
        call op_mpi_wait_all(size(args), args)
    end if

    call op_profile_exit()

    call op_mpi_set_dirtybit(size(args), args)
    call op_profile_exit()
end subroutine

end module

module op2_m_airfoil_2_adt_calc_fb

    use iso_c_binding

    use op2_fortran_declarations
    use op2_fortran_rt_support

    use op2_consts

    implicit none

    private
    public :: op2_k_airfoil_2_adt_calc_fb

contains

SUBROUTINE adt_calc(x1, x2, x3, x4, q, adt)
  REAL(KIND = 8), DIMENSION(2), INTENT(IN) :: x1, x2, x3, x4
  REAL(KIND = 8), DIMENSION(4), INTENT(IN) :: q
  REAL(KIND = 8), INTENT(OUT) :: adt
  REAL(KIND = 8) :: dx, dy, ri, u, v, c
  ri = 1.0_8 / q(1)
  u = ri * q(2)
  v = ri * q(3)
  c = SQRT(op2_const_gam * op2_const_gm1 * (ri * q(4) - 0.5_8 * (u ** 2 + v ** 2)))
  dx = x2(1) - x1(1)
  dy = x2(2) - x1(2)
  adt = ABS(u * dy - v * dx) + c * SQRT(dx ** 2 + dy ** 2)
  dx = x3(1) - x2(1)
  dy = x3(2) - x2(2)
  adt = adt + ABS(u * dy - v * dx) + c * SQRT(dx ** 2 + dy ** 2)
  dx = x4(1) - x3(1)
  dy = x4(2) - x3(2)
  adt = adt + ABS(u * dy - v * dx) + c * SQRT(dx ** 2 + dy ** 2)
  dx = x1(1) - x4(1)
  dy = x1(2) - x4(2)
  adt = adt + ABS(u * dy - v * dx) + c * SQRT(dx ** 2 + dy ** 2)
  adt = adt / op2_const_cfl
END SUBROUTINE

subroutine op2_k_airfoil_2_adt_calc_wr( &
    dat0, &
    dat1, &
    dat2, &
    map0, &
    n_exec, &
    set, &
    args &
)
    implicit none

    ! parameters
    real(8), dimension(:, :) :: dat0
    real(8), dimension(:, :) :: dat1
    real(8), dimension(:, :) :: dat2

    integer(4), dimension(:, :) :: map0

    integer(4) :: n_exec
    type(op_set) :: set
    type(op_arg), dimension(6) :: args

    ! locals
    integer(4) :: n

    do n = 1, n_exec
        if (n == set%setptr%core_size + 1) then
            call op_profile_next("MPI Wait")
            call op_mpi_wait_all(size(args), args)
            call op_profile_next("Computation")
        end if

        call adt_calc( &
            dat0(:, map0(1, n) + 1), &
            dat0(:, map0(2, n) + 1), &
            dat0(:, map0(3, n) + 1), &
            dat0(:, map0(4, n) + 1), &
            dat1(:, n), &
            dat2(1, n) &
        )
    end do
end subroutine

subroutine op2_k_airfoil_2_adt_calc_fb( &
    name, &
    set, &
    arg0, &
    arg1, &
    arg2, &
    arg3, &
    arg4, &
    arg5 &
)
    implicit none

    ! parameters
    character(kind=c_char, len=*) :: name
    type(op_set) :: set

    type(op_arg) :: arg0
    type(op_arg) :: arg1
    type(op_arg) :: arg2
    type(op_arg) :: arg3
    type(op_arg) :: arg4
    type(op_arg) :: arg5

    ! locals
    type(op_arg), dimension(6) :: args

    integer(4) :: n_exec

    real(8), pointer, dimension(:, :) :: dat0
    real(8), pointer, dimension(:, :) :: dat1
    real(8), pointer, dimension(:, :) :: dat2

    integer(4), pointer, dimension(:, :) :: map0

    real(4) :: transfer

    args(1) = arg0
    args(2) = arg1
    args(3) = arg2
    args(4) = arg3
    args(5) = arg4
    args(6) = arg5

    call op_profile_enter_kernel("airfoil_2_adt_calc", "seq", "Indirect")

    call op_profile_enter("MPI Exchanges")
    n_exec = op_mpi_halo_exchanges(set%setcptr, size(args), args)

    call op_profile_next("Computation")

    call c_f_pointer(arg0%data, dat0, (/2, getsetsizefromoparg(arg0)/))
    call c_f_pointer(arg4%data, dat1, (/4, getsetsizefromoparg(arg4)/))
    call c_f_pointer(arg5%data, dat2, (/1, getsetsizefromoparg(arg5)/))

    call c_f_pointer(arg0%map_data, map0, (/getmapdimfromoparg(arg0), set%setptr%size/))

    call op2_k_airfoil_2_adt_calc_wr( &
        dat0, &
        dat1, &
        dat2, &
        map0, &
        n_exec, &
        set, &
        args &
    )

    call op_profile_next("MPI Wait")
    if ((n_exec == 0) .or. (n_exec == set%setptr%core_size)) then
        call op_mpi_wait_all(size(args), args)
    end if

    call op_profile_exit()

    call op_mpi_set_dirtybit(size(args), args)
    call op_profile_exit()
end subroutine

end module

module op2_m_airfoil_2_adt_calc

    use iso_c_binding

    use op2_fortran_declarations
    use op2_fortran_rt_support

    use op2_m_airfoil_2_adt_calc_fb
    use op2_m_airfoil_2_adt_calc_m

    implicit none

    private
    public :: op2_k_airfoil_2_adt_calc

contains

subroutine op2_k_airfoil_2_adt_calc( &
    name, &
    set, &
    arg0, &
    arg1, &
    arg2, &
    arg3, &
    arg4, &
    arg5 &
)
    character(kind=c_char, len=*) :: name
    type(op_set) :: set

    type(op_arg) :: arg0
    type(op_arg) :: arg1
    type(op_arg) :: arg2
    type(op_arg) :: arg3
    type(op_arg) :: arg4
    type(op_arg) :: arg5

    if (op_check_whitelist("airfoil_2_adt_calc")) then
        call op2_k_airfoil_2_adt_calc_m( &
            name, &
            set, &
            arg0, &
            arg1, &
            arg2, &
            arg3, &
            arg4, &
            arg5 &
        )
    else
        call op_check_fallback_mode("airfoil_2_adt_calc")
        call op2_k_airfoil_2_adt_calc_fb( &
            name, &
            set, &
            arg0, &
            arg1, &
            arg2, &
            arg3, &
            arg4, &
            arg5 &
        )
    end if

end subroutine

end module